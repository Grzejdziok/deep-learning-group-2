...
if __name__ == "__main__":
    ...
    TOTAL_ITERATIONS = np.arange(0, 50001, 100)
    ...
    for pm in PM:
        while iterations_so_far < TOTAL_ITERATIONS:
            ...
            amount_to_prune = iterations_so_far
            prune.random_unstructured(model.layers[0], name="weight", amount=0.3)
            prune.random_unstructured(model.layers[2], name="weight", amount=0.3)
            prune.random_unstructured(model.layers[4], name="weight", amount=0.3)
