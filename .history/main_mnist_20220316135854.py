        ...
        for pm in PM:                               #These are for the colours of different fig 3 lines
            for i in range(pm):                   #This is for how many times you need to prune and train, per fig 3 line
                ...
                while iterations_so_far < total_iterations:
                    ...
                    weights_original = model.layers[0].weight
                    model.train()
                ...
                model.layers[0].weight = model.layers[0].weight.to_sparse() #This should keep pruned weights 0 #https://github.com/pytorch/tutorials/pull/605#issuecomment-585994076
                print(model.layers[0].weight)
